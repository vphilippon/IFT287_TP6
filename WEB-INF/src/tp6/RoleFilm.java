...
public class RoleFilm {
    ...
    public List<TuplePersonne> getActeurs(String titre, Date anneeSortie)
            throws SQLException {
        ...
        ResultSet rs = stmtGetActeurOfFilm.executeQuery();
        while (rs.next()) {
            listePersonne.add(new TuplePersonne(rs.getString(1), rs.getDate(2),
    ...
    public List<TupleRoleFilm> rolesDeActeur(String nom) throws SQLException {
        ...
        while (rs.next()) {
            listeRole.add(new TupleRoleFilm(rs.getString(1), rs.getString(2),
