...
public class Serie {
    ...
    private void init() throws SQLException {
        ...
        stmtAjoutDescSerie = cx.getConnection().prepareStatement(
                "UPDATE Serie SET description=? WHERE titre = ? AND anneesortie = ?");
    ...
    public List<TupleSerie> serieDeRealisateur(String nom) throws SQLException {
        ...
        while (rs.next()) {
            listeSerie.add(new TupleSerie(rs.getString(1), rs.getDate(2),
    ...
    public void ajouterDescription(String titre, Date anneeSortie, String description)
            throws SQLException {
        stmtAjoutDescSerie.setString(1, description);
        stmtAjoutDescSerie.setString(2, titre);
        stmtAjoutDescSerie.setDate(3, anneeSortie);
        stmtAjoutDescSerie.executeUpdate();
    }
    ...
    public TupleSerie getSerie(String titre, Date anneeSortie) throws SQLException {
        ...
        laSerie = new TupleSerie(rs.getString("titre"), rs.getDate("anneeSortie"),
                rs.getString("realisateur"), rs.getString("description"),
                rs.getInt("nbSaison"));
        rs.close();
        return laSerie;
    }
